...
<div class="modal fade" id="exampleModalDefault" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      الإحتمالات المتوفرة (60 شدة -325 شدة - 660 شدة ) <br>
    ...
    <div class="bgluky container">
              ...
              @else
               ...
               <div class="navbar-botton">
		               <span class="aaa"> <a href="./pubgguess"> عودة للخلف </a> </span>
	             </div>
              @endif
              </div>
              @if(!session()->has('backto'))
      <form class="needs-validation" novalidate action="{{route('createwinnerp')}}" method="post">
        @csrf 
        <div class="row g-3 my-3">
          <div class="col-md-12">
          <label for="idpubg" class="form-label" style="font-size:14px;"> اكتب  اي دي ببجي بشكل صحيح لتصلك الهدية </label>
          <input type="hidden" name="uccount" value="{{session()->get('uccount')}}">
          <input type="hidden" name="uccode" value=" {{ Session::get('uccode') }} ">
          <input type="hidden" name="guessno" value=" {{ Session::get('guessno') }} ">
          <input pattern="\d*" style="font-size:14px; text-align:left;" type="text" class="form-control" id="idpubg" name="idpubg" placeholder="اي دي ببجي" required oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*?)\..*/g, '$1');"  class="@error('idpubg') is-invalid @enderror">
          <div class="invalid-feedback">
          اكتب  اي دي ببجي لاستلام الهدية 
          </div>
          @error('idpubg')
      ...
      @endif
              ...
              @else
        <form class="needs-validation" novalidate action="{{route('guesscreate')}}" method="post">
                @csrf 
        <div class="row g-3 my-3">
            <div class="col-md-12">
              <label for="guessno" class="form-label" style="font-size:12px;">خمن رقم من 5 خانات واربح شدات ببجي</label>
